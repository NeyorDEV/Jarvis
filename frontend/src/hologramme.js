...
class HandTracker {
  ...
  async start() {
    ...
    try {
      ...
      // Camera utility MediaPipe : envoie CHAQUE frame sans throttle
      // Crucial pour la détection fiable des 2 mains simultanément
      if (typeof Camera !== 'undefined') {
        this._camera = new Camera(this.video, {
          onFrame: async () => {
            if (!this.app._running) return;
        ...
        // Fallback sans throttle si Camera utility absent
  ...
  stop() {
    if (this._camera) { try { this._camera.stop(); } catch(e) {} this._camera = null; }
...
class GestureSystem {
  constructor() {
    this.history = [[], []];
    this.indexHistory = [[], []];
    this.openFlag = [false, false];
    this.fistFlag = [false, false];
    this.lastMicToggleTime = 0;
  }
  ...
  pushIndexHistory(handIdx, hand) {
    if (!hand) { this.indexHistory[handIdx].length = 0; return; }
    const indexTip = hand[8];
    const now = performance.now();
    this.indexHistory[handIdx].push({ x: indexTip.x, y: indexTip.y, t: now });
    while (this.indexHistory[handIdx].length > 0 && now - this.indexHistory[handIdx][0].t > 2500) {
      this.indexHistory[handIdx].shift();
    }
  }
  detectCircle(handIdx) {
    const points = this.indexHistory[handIdx];
  ...
  detectSwipe(handIdx) {
    ...
    const now = performance.now();
    // Garde l'historique propre des entrées de plus de 500ms
    while (hist.length > 0 && now - hist[0].t > 500) {
      hist.shift();
    }
    ...
    for (let i = 0; i < hist.length - 3; i++) {
      ...
      if (dt > 400) continue; // Seulement les mouvements rapides (< 400ms)
      
      const dx = newest.x - old.x;
      const dy = newest.y - old.y;
      
      if (Math.abs(dx) > 0.16 && Math.abs(dx) > Math.abs(dy) * 1.5) {
        hist.length = 0;
...
class UIManager {
  ...
  _bindDock() {
    ...
    this.container.querySelector('#holo-btn-explorer')?.addEventListener('click', () => {
      this.app.toggleExplorer();
    });
    this.container.querySelector('#holo-btn-domotic')?.addEventListener('click', () => {
      this.app.toggleDomotic();
    });
...
class App {
  constructor(container) {
    ...
    this.renderer.scene.add(this.shapesGroup);
    this.shapes      = new ShapeManager(this.shapesGroup);
    ...
    this._onMouseMove = (e) => {
      ...
      if (this._isLeftDown) {
        // Déplacement/Translation de toute la scène de formes
        this.shapesGroup.position.x += dx * 0.006;
        this.shapesGroup.position.y -= dy * 0.006;
      } else if (this._isRightDown) {
        // Rotation de toute la scène de formes
        this.shapesGroup.rotation.y += dx * 0.006;
        this.shapesGroup.rotation.x += dy * 0.006;
        // Limiter la rotation X pour éviter de retourner le groupe
        this.shapesGroup.rotation.x = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, this.shapesGroup.rotation.x));
      }
    };
  ...
  toggleExplorer() {
    const btn = this.ui.container.querySelector('#holo-btn-explorer');
    if (this.spatialExplorer?.active) {
      this.spatialExplorer.deactivate();
      this.spatialExplorer = null;
      window._spatialExplorer = null;
      ...
      if (!ws) { console.warn('[SPATIAL] WebSocket non disponible'); return; }
      const SFE = window.SpatialFileExplorer;
      if (!SFE) { console.warn('[SPATIAL] Module non chargé'); return; }
      this.shapes.shapes.forEach(s => { s.group.visible = false; });
      this.spatialExplorer = new SFE(this.renderer.scene, this.renderer.camera, ws);
      window._spatialExplorer = this.spatialExplorer;
      this.spatialExplorer.activate();
      this.ui.showGesture('EXPLORER ON');
      if (btn) btn.classList.add('active');
    }
  }

  toggleDomotic() {
    const btn = this.ui.container.querySelector('#holo-btn-domotic');
    if (this.domoticMap?.active) {
      this.domoticMap.deactivate();
      this.domoticMap = null;
      window._domoticMap = null;
      this.shapes.shapes.forEach(s => { s.group.visible = true; });
      ...
      const ws = window._jarvisWs;
      if (!ws) { console.warn('[DOMOTIC] WebSocket non disponible'); return; }
      const DM = window.DomoticMap;
      if (!DM) { console.warn('[DOMOTIC] Module non chargé'); return; }
      this.shapes.shapes.forEach(s => { s.group.visible = false; });
      this.domoticMap = new DM(this.renderer.scene, this.renderer.camera, ws);
      window._domoticMap = this.domoticMap;
      this.domoticMap.activate();
      this.ui.showGesture('DOMOTIQUE ON');
      if (btn) btn.classList.add('active');
    }
  }

  toggleCortex() {
    const btn = this.ui.container.querySelector('#holo-btn-cortex');
    if (this.cortexMap?.active) {
      this.cortexMap.deactivate();
      this.cortexMap = null;
      window._cortexMap = null;
      this.shapes.shapes.forEach(s => { s.group.visible = true; });
      this.ui.showGesture('CORTEX OFF');
      if (btn) btn.classList.remove('active');
    } else {
      if (this.spatialExplorer?.active) {
        this.toggleExplorer();
      ...
      const ws = window._jarvisWs;
      if (!ws) { console.warn('[CORTEX] WebSocket non disponible'); return; }
      const CM = window.CortexMap;
      if (!CM) { console.warn('[CORTEX] Module non chargé'); return; }
      this.shapes.shapes.forEach(s => { s.group.visible = false; });
      this.cortexMap = new CM(this.renderer.scene, this.renderer.camera, ws);
      window._cortexMap = this.cortexMap;
      this.cortexMap.activate();
      this.ui.showGesture('CORTEX ON');
      if (btn) btn.classList.add('active');
    }
  }
  ...
  _processGestures(dt) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      // Enregistrement inconditionnel de la trajectoire de l'index
      this.gestures.pushIndexHistory(i, h);
      
      // Détection inconditionnelle du geste circulaire (Circular Wake)
      if (h) {
        const now = performance.now();
        if (now - this.gestures.lastMicToggleTime > 2000) {
          if (this.gestures.detectCircle(i)) {
            this.gestures.lastMicToggleTime = now;
            this.gestures.indexHistory[i].length = 0; // Clear history
            
            const ws = window._jarvisWs;
            if (ws && ws.readyState === WebSocket.OPEN) {
              ws.send(JSON.stringify({ type: "toggle_mic" }));
            }
            this.ui.showGesture('MIC TOGGLE');
          }
        }
      }
      ...
      if (!h) { 
        ...
        this.handPos3D[i] = null; 
        this.openFlag[i] = false; 
        this.fistFlag[i] = false; 
        continue; 
      }
      ...
      if (this.renderer.pointers[i]) { 
        this.renderer.pointers[i].visible = true; 
        this.renderer.pointers[i].position.copy(this.handPos3D[i]); 
        this.renderer.pointers[i].scale.setScalar(pinched ? 0.6 : 1.0); 
      }

      // Simulation de clic virtuel sur le dock (HTML) lors d'un pincement de doigts
      if (pinched && !this.wasPinched[i]) {
        this.wasPinched[i] = true;
        const screenPos = this._getScreenCoords(h[8]); // Bout de l'index
        const oldPointer = this.handCanvas.style.pointerEvents;
        ...
        this.handCanvas.style.pointerEvents = oldPointer;
        this.threeCanvas.style.pointerEvents = oldPointer;
      ...
      if (pinched) {
        if (!this.handGrab[i]) {
          ...
          if (v) { v.shape.grabVertex(v.vertexIdx, i); this.handGrab[i] = { shape: v.shape, vertexIdx: v.vertexIdx }; }
    ...
    if (isExplorerActive) {
      ...
      this.spatialExplorer.updateHandsInteraction(p0, pinched0, p1, pinched1);
    ...
    if (isDomoticActive) {
      ...
      this.domoticMap.updateHandsInteraction(p0, pinched0, p1, pinched1);
    ...
    if (isCortexActive) {
      ...
      this.cortexMap.updateHandsInteraction(p0, pinched0, p1, pinched1);
    ...
    // Double-hand swipe detection for clearing all shapes
    if (hands.length === 2) {
      const swipe0 = this.gestures.detectSwipe(0);
      const swipe1 = this.gestures.detectSwipe(1);
      if (swipe0 && swipe1) {
        this.clearShapes();
        gestureMsg = 'CLEAR ALL';
      }
    }
