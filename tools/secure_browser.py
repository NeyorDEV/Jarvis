...
_broadcast_fn = None         # Injecté par main2 : send_web_broadcast_sync
...
def dock_browser():
    ...
    try:
        if _broadcast_fn:
            _broadcast_fn({"action": "browser_state", "state": "docked"})
...
def center_and_own_browser():
    ...
    try:
        if _broadcast_fn:
            _broadcast_fn({"action": "browser_state", "state": "undocked"})
    except Exception as e:
        print(f"[BROWSER] Error broadcasting state: {e}")
...
def _on_browser_closed():
    ...
    try:
        if _broadcast_fn:
            _broadcast_fn({"action": "browser_state", "state": "closed"})
    except Exception as e:
        print(f"[BROWSER] Error broadcasting state: {e}")
