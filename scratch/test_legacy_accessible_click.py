...
async def main():
    ...
    print(f"Active window before LegacyIAccessible Click: {active_before} (title: '{win32gui.GetWindowText(active_before)}')")

    # Step 4: Perform LegacyIAccessible DoDefaultAction
    print("Performing LegacyIAccessible DoDefaultAction...")
    try:
        pattern = found_btn.GetLegacyIAccessiblePattern()
        if pattern:
            pattern.DoDefaultAction()
            print("LegacyIAccessible DoDefaultAction called successfully.")
        else:
            print("❌ LegacyIAccessiblePattern not supported on this button.")
    except Exception as e:
        print(f"❌ LegacyIAccessiblePattern failed: {e}")
    
    time.sleep(3.0)
    
    active_after = win32gui.GetForegroundWindow()
    print(f"Active window after LegacyIAccessible Click: {active_after} (title: '{win32gui.GetWindowText(active_after)}')")
    
    if active_after == active_before:
        print("✅ SUCCESS: LegacyIAccessible Click worked without stealing focus!")
    else:
        print("❌ FAILURE: Focus was stolen by LegacyIAccessible Click!")
