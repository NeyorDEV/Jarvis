    ...
    for hwnd in hwnds:
        try:
            ...
            if not (style & 0x08000000):
                ...
                print(f"Applied WS_EX_NOACTIVATE to HWND {hwnd}")
...
async def main():
    ...
    print(f"Active window before URI open: {active_before} (title: '{win32gui.GetWindowText(active_before)}')")

    # Open URI
    print("Opening playlist URI in background...")
    _ouvrir_uri_deezer("deezer://playlist/3110452282")
    
    time.sleep(4.0)
    
    active_after = win32gui.GetForegroundWindow()
    print(f"Active window after URI open: {active_after} (title: '{win32gui.GetWindowText(active_after)}')")
    
    if active_after == active_before:
        print("✅ SUCCESS: URI open did not steal focus!")
    else:
