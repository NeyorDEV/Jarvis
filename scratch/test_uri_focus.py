...
async def main():
    ...
    print(f"Active window before URI call: {active_before} ({win32gui.GetWindowText(active_before)})")
    
    print("\nCalling _ouvrir_uri_deezer inside prevent_focus_theft...")
    with prevent_focus_theft():
        # Restore window shownoactivate
        win32gui.ShowWindow(hwnd, win32con.SW_SHOWNOACTIVATE)
        _ouvrir_uri_deezer("deezer://album/1398927")
        time.sleep(3.0)
        
    active_after = win32gui.GetForegroundWindow()
    print(f"Active window after URI call: {active_after} ({win32gui.GetWindowText(active_after)})")
    print(f"Is Deezer visible? {win32gui.IsWindowVisible(hwnd)}")
    
    if active_after == active_before:
        print("SUCCESS: No focus theft occurred!")
    else:
